...
## 6. Use Flyway for Database Migrations
* Place migration scripts in the default directory: `src/main/resources/db/migration`.
* Follow the versioned naming convention: `V{version}__{description}.sql` (e.g., `V1__create_users_table.sql`, `V2__add_email_column.sql`).
* Use sequential version numbers (V1, V2, V3) for ordered execution.
* Include descriptive names separated by underscores after the double underscore.
* Keep migrations immutable once they've been applied to any environment.

**Explanation:**

* **Automated schema evolution:** Flyway automatically applies database changes in the correct order, ensuring consistent schema across all environments.
* **Version control for your database:** Migration scripts are versioned and tracked alongside application code, providing a complete history of database changes.
* **Repeatable migrations:** For scripts that can be re-run (like views or stored procedures), use the `R__` prefix instead of `V` (e.g., `R__create_summary_view.sql`).
* **Consistent environments:** The same migrations run in development, testing, and production, eliminating "works on my machine" database issues.

## 7. Separate Web Layer from Persistence Layer
...
## 8. Follow REST API Design Principles
...
## 9. Use Command Objects for Business Operations
...
## 13. Use Testcontainers for integration tests
* Spin up real services (databases, message brokers, etc.) in your integration tests to mirror production environments.
* Use the Spring Boot Testcontainers integration for seamless container lifecycle management.
* Define container configurations as static fields in your test classes or in a shared configuration class.
...
* Spring Boot's Testcontainers integration automatically manages container lifecycle and provides property sources for connection details:
...
## 14. Use random port for integration tests
