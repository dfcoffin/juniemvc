...
describe("BeerService", () => {
  ...
  describe("getBeers", () => {
    it("fetches beers with default parameters", async () => {
      ...
      // Verify
      expect(apiService.get).toHaveBeenCalledWith("/api/v1/beers", {
        params: {
          pageNumber: 0,
          pageSize: 25,
          showInventory: false,
  ...
  describe("getBeerById", () => {
    it("fetches a beer by ID", async () => {
      ...
      // Verify
      expect(apiService.get).toHaveBeenCalledWith("/api/v1/beers/1");
    ...
    it("handles API errors", async () => {
      ...
      // Execute and verify
      await expect(BeerService.getBeerById("999")).rejects.toThrow(
        errorMessage,
      );
    });
  });
  ...
  describe("createBeer", () => {
    it("creates a new beer", async () => {
      ...
      // Verify
      expect(apiService.post).toHaveBeenCalledWith("/api/v1/beers", newBeer);
  ...
  describe("deleteBeer", () => {
    it("deletes a beer", async () => {
      ...
      // Verify
      expect(apiService.delete).toHaveBeenCalledWith(`/api/v1/beers/${beerId}`);
    });
  });
});
