...
function checkCoverage(summary: any) {
  ...
  if (total.lines.pct < COVERAGE_THRESHOLDS.lines) {
    failures.push(`Line coverage ${total.lines.pct}% is below threshold ${COVERAGE_THRESHOLDS.lines}%`);
  }
  
  if (total.statements.pct < COVERAGE_THRESHOLDS.statements) {
    failures.push(`Statement coverage ${total.statements.pct}% is below threshold ${COVERAGE_THRESHOLDS.statements}%`);
  }
  
  if (total.functions.pct < COVERAGE_THRESHOLDS.functions) {
    failures.push(`Function coverage ${total.functions.pct}% is below threshold ${COVERAGE_THRESHOLDS.functions}%`);
  }
  
  if (total.branches.pct < COVERAGE_THRESHOLDS.branches) {
    failures.push(`Branch coverage ${total.branches.pct}% is below threshold ${COVERAGE_THRESHOLDS.branches}%`);
  }
  
  // Check specific directories
  for (const dir of COVERAGE_DIRECTORIES) {
    // Find all entries that match this directory
    const dirEntries = Object.entries(summary)
      .filter(([key]) => key !== 'total' && key.startsWith(dir));
    ...
    for (const [, data] of dirEntries) {
      if ((data as any).lines && typeof (data as any).lines.pct === 'number') {
        lineTotal += (data as any).lines.pct;
        lineCount++;
      }
    }
    ...
    if (avgLineCoverage < COVERAGE_THRESHOLDS.lines) {
      failures.push(`Average line coverage for ${dir} is ${avgLineCoverage.toFixed(2)}%, below threshold ${COVERAGE_THRESHOLDS.lines}%`);
    }
  }
...
function generateCoverageReport(summary: any) {
  console.log('\n========== COVERAGE REPORT ==========\n');
  ...
  // Group by directory
  const directoryCoverage: Record<string, any[]> = {};
