...
export const mockBeerService = {
  ...
  patchBeer: vi
    .fn()
    .mockImplementation((id: string, beerPatchDto: Partial<BeerDto>) => {
      const beer = mockBeers.find((b) => b.id === id);
      if (!beer) {
        return Promise.reject(new Error("Beer not found"));
      }
  ...
  uploadBeerImage: vi.fn().mockImplementation((id: string, _file: File) => {
    const beer = mockBeers.find((b) => b.id === id);
    if (!beer) {
      return Promise.reject(new Error("Beer not found"));
    }
...
export const mockCustomerService = {
  ...
  createCustomer: vi
    .fn()
    .mockImplementation((customerDto: Partial<Customer>) => {
  ...
  updateCustomer: vi
    .fn()
    .mockImplementation((id: number, customerDto: Partial<Customer>) => {
...
export const mockBeerOrderService = {
  ...
  updateBeerOrder: vi
    .fn()
    .mockImplementation((id: number, orderDto: BeerOrderDto) => {
  ...
  updateBeerOrderStatus: vi
    .fn()
    .mockImplementation((id: number, status: BeerOrderStatus) => {
      const order = mockBeerOrders.find((o) => o.id === id);
      if (!order) {
        return Promise.reject(new Error("Order not found"));
      }
  ...
  createBeerOrderShipment: vi.fn().mockImplementation(
    (
      orderId: number,
      shipmentData: {
        shipmentDate: string;
